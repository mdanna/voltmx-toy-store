define(['./Inherits', './NativeController'], function(Inherits, NativeController) {
	...
	NativeControllerIOS.prototype.importClasses = function() {
		try {
			...
			this.recognizerDelegate = objc.newClass('recognizerDelegate' + this.getRandomNumber(), 'NSObject', ['SFSpeechRecognizerDelegate'], {
			...
			this.recognizerDelegateObj = this.recognizerDelegate.jsnew();
		...
		try {
			...
			if (this._isValidLanguage) {
				...
				if (this.recognizer !== undefined && this.recognizer !== null) {                    
					this.recognizerDelegate = this.recognizerDelegateObj;
					this.recognizer.delegate = this.recognizerDelegate;
	...
	NativeControllerIOS.prototype.iosStartListening = function() {
		try {
			...
			if (!this.audioEngine.running) {                
				...
				this.inputNode.installTapOnBusBufferSizeFormatBlock(this.componentInstance._avAudioBus, this.componentInstance._bufferSize, this.inputNode.outputFormatForBus(this.componentInstance._avAudioBus), function(buf, when) {
				...
				konymp.logger.info('Audio Engine is running!');
			}
